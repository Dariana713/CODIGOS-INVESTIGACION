...
setwd("~/Codigos Inves_DIAV/CARPETA DE TRABAJO ACTUALIZADA AL DIA/Unión para hacer un rds y correlaciones/NDVI/12_ETN ESP")
ETn <- stack(list.files(pattern = ".tif"))

setwd("~/Codigos Inves_DIAV/CARPETA DE TRABAJO ACTUALIZADA AL DIA/Unión para hacer un rds y correlaciones/NDVI/36N dias de NDVI con mascara de cobertura")
NDVI <- stack(list.files(pattern = ".tif"))

rstack7 <- stack(ETn, NDVI)
plot(rstack7)
...
df01 <- data.frame( Corrpos=Canpos1, corrnes=Canneg1, AreaTotal=areatotal, variable="Correlacion_mas_menos01ET_NDVI")
...
df02 <- data.frame( Corrpos=Canpos2, corrnes=Canneg2, AreaTotal=areatotal, variable="Correlacion_mas_menos03PreETET_NDVI")
...
df03 <- data.frame( Corrpos=Canpos3, corrnes=Canneg3, AreaTotal=areatotal, variable="Correlacion_mas_menos05PreETET_NDVI")
...
df04 <- data.frame( Corrpos=Canpos4, corrnes=Canneg4, AreaTotal=areatotal, variable="Correlacion_mas_menos07PreETET_NDVI")
...
df05 <- data.frame( Corrpos=Canpos5, corrnes=Canneg5, AreaTotal=areatotal, variable="Correlacion_mas_menos09PreETET_NDVI")

unionTabla8 <- rbind(df01, df02, df03, df04, df05)
unionTabla8 <-unionTabla8[c(-2, -4, -6, -8, -10),]
write.csv(unionTabla8, file ="Correlaciones_anuales12ET_36NDVI_2daiteracion.csv")
