...
rstack8 <- stack(temp, ETn)
plot(rstack8)


system.time(cg9 <- gridcorts(rasterstack = rstack8, method = "pearson", type = "both"))
...
unionTabla9 <- rbind(df01, df02, df03, df04, df05)
unionTabla9 <-unionTabla9[c(-2, -4, -6, -8, -10),]
write.csv(unionTabla9, file ="Correlaciones_anuales12TEMP_12ET_ND.csv")
