...
#Correlacion de SSM y FAPAR revisar el orden de X (ssm) ; Y(fapar) POSITIVO
dat <- readRDS("~/Análisis de Tesis en Rstudio y SAGA GIS/Codigos de estadisticas/CARPETA DE TRABAJO ACTUALIZADA AL DIA/Unión para hacer un rds y correlaciones/FAPAR/base_completa_fapar_ssm_pre_ET_Tem_topoPOSITIVO.rds")
# mean <- mean(corrn)
#[1] 0.1188747
#max
#[1] 0.4059697
...
datMeanSSM <- colMeans(dat@data[37:72])
datmeanFAPAR <- colMeans(dat@data[1:36])
...
dat <- readRDS("~/Análisis de Tesis en Rstudio y SAGA GIS/Codigos de estadisticas/CARPETA DE TRABAJO ACTUALIZADA AL DIA/Unión para hacer un rds y correlaciones/FAPAR/base_completa_fapar_ssm_pre_ET_Tem_topoNEGATIVO.rds")
corrn <- numeric()
for (i in 1:36){
  df <- na.omit(data.frame(x=dat@data[,36+i], y=dat@data[,i]))
  corrn[i] <- cor(df$x, df$y)
...
resn <- data.frame(Días=1:36, Correlación = corrn)
plot(resn, col='black', ylim=c(-0.8, 0.8), main="Correlación SSM y FAPAR")
lines(resn)
abline(h=0)
mapview(dat)
...
summary(dat@data[,c(1,37)])
cor(na.omit(dat@data[,c(1,37)]))
dat$meanSM <- colMean(dat@data[37:72])
dat$meanNDVI <- colMean(dat@data[1:36])
